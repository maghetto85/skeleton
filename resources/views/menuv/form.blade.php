...
@section('page.navbar')
    <li><a href="{{ route('menuv.index') }}"><i class="fa fa-chevron-left fa-fw"></i> Menu</a></li>
@endsection
@section('head')
...
<div class="container">
    ...
    <form method="post" action="{{ $menu->exists ? route('menuv.update', $menu->id) : route('menuv.store') }}">
        {{ csrf_field() }}
        @if($menu->exists)
            {{ method_field('put') }}
        @endif
