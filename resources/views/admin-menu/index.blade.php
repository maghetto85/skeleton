...
<div class="container">
     ...
     <div style="margin-bottom: 20px;">
        ...
        <a href="{{ route('admin-menu.create') }}" class="btn btn-primary btn-sm">Nuovo Elemento</a>
    </div>
    ...
    <div class="panel panel-default">
        ...
        <div class="table-responsive">
            <table class="table table-bordered table-striped table-condensed">
                ...
                <tbody>
                @forelse($adminmenu as $menuitem)
                <tr>
                    <td class="text-center success">
                        <a href="{{ route('admin-menu.edit', $menuitem->id) }}"><i class="fa fa-fw fa-pencil"></i></a>
                    </td>
                    <td class="text-center danger">
                        <a href="#elimina" data-toggle="modal" data-id="{{ $menuitem->id }}" data-name="{{ $menuitem->titolo }}"><i class="fa fa-fw fa-remove"></i></a>
                    </td>
                    <td>{{ $menuitem->titolo }}</td>
                    <td>{{ $menuitem->url }}</td>
                    <td><i class="fa fa-fw fa-{{ $menuitem->visibile ? 'check text-success' : 'remove text-danger' }}"></i></td>
                </tr>
                @empty
                <tr>
...
@section('bottom')
    ...
    <script>
        ...
        $('#elimina').on('show.bs.modal', function (event) {
            ...
            var modal = $(this)
            modal.find('form').attr('action','{{ route('admin-menu.destroy',null) }}/'+id);
