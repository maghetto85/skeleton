	...
	$.fn.TMForm=function(opt){
		...
		function TMForm(){
			...
			function formSubmit(){
				...
				$('#contact-form label').each(function (id, label) {

					data[$(label).attr('class').replace(' ok','')] = getValue($(label).find('select, input, textarea'));

				});

				data._token = Laravel.csrfToken;

				data.owner_email = opt.ownerEmail || '#';
				data.stripHTML = opt.stripHTML;
...
regula.custom({
	...
	,validator:function(){
		...
		return !(isNaN(string2Date(this.value, 'MM/dd/yyyy').getYear()) &&
						isNaN(string2Date(this.value, 'dd/MM/yyyy').getYear()) &&
						isNaN(string2Date(this.value, 'yyyy-MM-dd').getYear()))
	}
})

$(window).load(function(){
        $('#contact-form:not(.prenotazioni)').TMForm({
            ownerEmail:'info@halex.it'
        });

		$('#contact-form.prenotazioni').TMForm({
            mailHandlerURL:'/prenotazioni',
			successShowDelay:'5550000'
